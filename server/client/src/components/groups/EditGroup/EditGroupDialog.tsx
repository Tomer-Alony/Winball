...
export default function UserData(props: EditGroupProps, state: EditGroupState) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const playersPromise = axios.get("/api/players/all");
      const leaguesPromise = axios.get("/api/leagues/");
      ...
      if (responses.length === 3) {
        setUsersData(
          responses[0].data.filter((user) => user._id !== responses[2].data._id)
        );
  ...
  const handleSave = async () => {
    ...
    const updatedPlayers = group.players.filter((player) => {
      return (
        player.playerId.toString() === loggedUser._id ||
        ...
        <DialogContent>
          {isLoading ? (
            ...
            <form className={classes.root}>
              ...
              />
              <DialogContentText>Group participants:</DialogContentText>
              <DataSelection
                data={usersData}
              ...
              />
              <DialogContentText>Group leagues:</DialogContentText>
              <DataSelection
                data={leaguesData}
